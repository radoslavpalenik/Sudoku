        ...
        # Herny grid
       	...
       	for rows in range(0,9):
            for columns in range(0,9):
                bgcol = "#2c3c43"
                if ((columns <= 2 or columns >= 6) and (rows <= 2 or rows >=6)) or ((columns > 2 and columns < 6 ) and (rows > 2 and rows < 6)) :
                    bgcol = "#4a6571"
                sdkBtn[rows][columns] = tk.Button(playMatrix, bd = 0,text = fullBoard[rows][columns],width = 5, height = 3, bg = bgcol,
                    fg = "#7aa719", font = ('Helvetica 15 bold') , highlightthickness = 0,
